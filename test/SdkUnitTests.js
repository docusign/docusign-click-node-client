...
describe('SDK Unit Tests:', function (done) {
  ...
  var scopes = [
    ...
    oAuth.Scope.CLICK_SEND,
    oAuth.Scope.CLICK_MANAGE
  ];
  ...
  before(function (done) {
    ...
    var oauthLoginUrl = apiClient.getJWTUri(integratorKey, RedirectURI, oAuthBasePath, scopes);
  ...
  it('getClickwraps', function (done) {
    var clickApi = new docusign.AccountsApi(apiClient);
    clickApi.getClickwraps(accountId, {})
      .then(function (data) {
        if (data) {
          console.log('Clickwraps: ' + JSON.stringify(data.clickwraps));
          assert.ok(data.clickwraps && data.clickwraps.length);
          done();
        }
      })
      .catch(function (error) {
